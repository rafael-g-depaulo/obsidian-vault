...
Default (STR)
```
&{template:default} {{name=@{selected|token_name} ataque (STR)}} [[ (( [[ceil(floor([[1d20cs>[[20-@{selected|wpn_margin}]] + @{selected|STR} + @{selected|proficiency}]]/(20-@{selected|wpn_margin} + @{selected|STR} + @{selected|proficiency}))/100)]]*(@{selected|wpn_mult}-1) + 1) * @{selected|wpn_dice})@{selected|wpn_dmg} + @{selected|STR} ]]. {{acerto= $[[1]] }} {{dano=$[[3]] }}
```

Default (DEX)
```
&{template:default} {{name=@{selected|token_name} ataque (DEX)}} [[ (( [[ceil(floor([[1d20cs>[[20-@{selected|wpn_margin}]] + @{selected|DEX} + @{selected|proficiency}]]/(20-@{selected|wpn_margin} + @{selected|DEX} + @{selected|proficiency}))/100)]]*(@{selected|wpn_mult}-1) + 1) * @{selected|wpn_dice})@{selected|wpn_dmg} + @{selected|DEX} ]]. {{acerto= $[[1]] }} {{dano=$[[3]] }}
```
